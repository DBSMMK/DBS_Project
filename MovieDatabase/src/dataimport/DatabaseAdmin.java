...
public class DatabaseAdmin {
	...
	public void createTables(){
		...
		try {
			...
			String sql =
				...
				"	lastname 		VARCHAR(127) NOT NULL,   \r\n" +
				"	movieID 		VARCHAR(9) NOT NULL, 	\r\n" +
				"	PRIMARY KEY (firstname,lastname, movieID), \r\n" + 
				"	FOREIGN KEY (movieID) REFERENCES movies(id),   \r\n" + 
				"	FOREIGN KEY (firstname,lastname) REFERENCES directors (firstname,lastname));\r\n" + 
				"			  \r\n" + 
	...
	public void crawlMovieData(String file) throws Exception{
		...
		//Maximale anzahl an insert Statement pro Anweisung 
		//(Bei zu langem Statement wirft Postgre einen Error)
		int commitSize = 5;
		...
		try {
			for (int i=0;(line = reader.readLine() ) != null;i++) {
				...
				String sql = "INSERT INTO movies (id,title,rel_year,rating,rating_count,duration) VALUES "+
	...
	private void insertDirector(String movieID, String[] name,Connection c){
		try {
			...
			sql += "INSERT INTO directors"+
							...
							"WHERE NOT EXISTS ("+
							" SELECT firstname,lastname FROM directors WHERE firstname = \'"
										+name[0]+"\' AND lastname = \'"+name[1]+"\'"+
							" ); ";
			sql += "INSERT INTO directs"+
					"(firstname,lastname,movieID)"+
					" SELECT '"+name[0]+"','"+name[1]+"','"+movieID+"'"+
					"WHERE NOT EXISTS ("+
					" SELECT firstname,lastname,movieID FROM directs WHERE firstname = '"
						+name[0]+"' AND lastname = '"+name[1]+"' AND movieID = '"+movieID+"' ); ";
		...
		try {
			...
			for (String fullname : fullnames){
				...
				sql += "INSERT INTO actors"+
						...
						" SELECT '"+name[0]+"', '"+name[1]+"'"+
						"WHERE NOT EXISTS ("+
						" SELECT firstname,lastname FROM actors WHERE firstname = '"
									+name[0]+"' AND lastname = '"+name[1]+"'"+
						" ); ";
				...
				" SELECT '"+name[0]+"','"+name[1]+"','"+movieID+"'"+
				"WHERE NOT EXISTS ("+
				" SELECT firstname,lastname,movieID FROM acts_in WHERE firstname = '"
				+name[0]+"' AND lastname = '"+name[1]+"' AND movieID = '"+movieID+"' ); ";
	...
	private void insertGenres(String movieID, String[] genres, Connection c){
		...
		try {
			...
			for (String genre : genres){
				...
				"WHERE NOT EXISTS ("+
				" SELECT movieID,genre FROM has_genre WHERE movieID = '"
								+movieID+"' AND genre = '"+genre+"'); ";
